...
/// use DashMap to build Memtable, implemented the Storage trait
...
impl MemTable {
    /// build a default MemTable
    pub fn new() -> Self {
        Self::default()
    }

    /// if table named name does not exist, create it, otherwise return
    pub fn get_or_create_table(&self, name: &str) -> Ref<String, DashMap<String, Value>> {
        match self.tables.get(name) {
            Some(table) => table,
            None => {
                let entry = self.tables.entry(name.into()).or_default();
                entry.downgrade() // TODO
            }
        }
    }
}
